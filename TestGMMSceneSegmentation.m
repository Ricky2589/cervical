function [ varargout ] = TestGMMSceneSegmentation( gmm_model_foreground, gmm_model_background, im_i)
...
%     imSize = 1024; %512;
    ...
    imTestFeaturesVector = [];
        ...
        imTestFeaturesVector.data = [ double( VecImGray ) / 255; ...
            double( VecImGradient) / double( max(VecImGradient) ); ...
            double( VecImLBP / 255 ) ];
    
    
    gmm_post = cell(length(im_i),1);

        % Test with training data
        prob_XY_foreground = pdfgauss( imTestFeaturesVector.data, gmm_model_foreground );
        prob_XY_background = pdfgauss( imTestFeaturesVector.data, gmm_model_background );    
        ...
        gmm_post = zeros(2, imSizeRow * imSizeCol);
        gmm_post(1, :) = post_Y_foreground(1,:);
        gmm_post(2, :) = post_Y_background(1,:);
